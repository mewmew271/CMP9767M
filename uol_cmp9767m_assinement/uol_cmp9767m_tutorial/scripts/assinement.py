#!/usr/bin/env python
#Andrew Henry 14552894@students.lincoln.ac.uk
# required 
	...
	def image_callback(self, data):#main loop 
			...
			if M["m10"] or M["m00"] or M["m01"] != 0:#error checking 
				...
				for t in coordsBeen:
					if t == objectCoords:
						...
						break           
				if same == False:
					for e in objectsLocated:
						if e  == objectCoords:
							same = True
							break 

				if same == False and abs(objectCoords[0]) < 5.5:
					objectsLocated.append(copy.copy(objectCoords))

			# 	for e in objectsLocated:
			# 		temp = [e]
			# 		for a in objectsLocated:
			# 			if e[0] - a[0] < 0.2 and e[1] - a[1] < 0.2:
			# 				temp.append(a)
			# 		objectsLocated.append(copy.copy(sum(temp)/len(temp)))

			# for t in objectsLocated:
			# 	cv2.circle(cv_image, (t[0], t[1]), 5, (255, 0, 0), -1)

		cv2.imshow("hsv", mask)
		cv2.imshow("window", cv_image)
		for x in range(len(objectsLocated)/10):
				# print'pop: ', x 
		...
		if len(objectsLocated) == 0:
			...
			if success:
				...
				# move coord to the end of list to move back to later
			...
			#success = False 
			back = true
			previous_position = currentLocation
			if success:
				rospy.loginfo("Hooray, reached the desired pose")
				coordsBeen.append(copy.copy(objectsLocated[0]))
				objectsLocated.pop(0)
				print'spray weed'
				if back == False:
					success = robot_main.goto(previous_position, quaternion)
