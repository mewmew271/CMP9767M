...
class image_projection:
    ...
    def image_callback(self, data):
        ...
        hsv_thresh = cv2.inRange(hsv_img,
            numpy.array((120, 100, 100)),
            numpy.array((120, 100, 0)))

        #hsv_image = hsv_img - hsv_thresh

        cv2.circle(cv_image, (int(uv[0]),int(uv[1])), 10, 255, -1)

        #resize for visualisation
        cv_image_s = cv2.resize(cv_image, (0,0), fx=0.5, fy=0.5)
        hsv_image_s = cv2.resize(hsv_thresh, (0,0), fx=0.5, fy=0.5)
        
        cv2.imshow("Image window", hsv_image_s)
        cv2.waitKey(1)
